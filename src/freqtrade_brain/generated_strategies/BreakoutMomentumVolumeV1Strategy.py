# Generated by Trading Research OS from YAML spec: breakout_momentum_volume_v1
...
class BreakoutMomentumVolumeV1Strategy(IStrategy):
    """
    Source YAML spec:
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = [(dataframe['close'] > dataframe['bb_upper']) & (dataframe['macd'] > dataframe['macd_signal']) & (dataframe['volume_zscore'] > 1.0)]
        if conditions:
            dataframe.loc[reduce(lambda left, right: left & right, conditions), "enter_long"] = 1
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = [(dataframe['close'] < dataframe['sma_20']) | (dataframe['rsi_14'] > 78)]
        if conditions:
